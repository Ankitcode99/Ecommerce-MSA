...
class CustomerRepository {
    ...
    try {
      ...
      if (profile) {
        ...
        if (cartItems.length > 0) {
          ...
          console.log("Updated cart items: " + cartItems, isExist)
          if (!isExist) {
        ...
        console.log("Testing",profile.cart);
  ...
  async AddOrderToProfile(customerId, order) {
    try {
      console.log("Logging inside repository")
      ...
      if (profile) {
        ...
        if (profile.orders == undefined || profile.orders.length == 0) {
          profile.orders = [];
        }
